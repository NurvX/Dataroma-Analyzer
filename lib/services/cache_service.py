...
class CacheService:
    ...
    def __init__(self, cache_dir: str = "cache"):
        ...
        self.activities_file = self.json_dir / "activities.json"  # New standard name
        self.activities_file_legacy = self.json_dir / "history.json"  # Legacy name for backward compatibility
    ...
    def save_activities(self, activities: List[Activity]) -> None:
        ...
        Writes to both activities.json (new standard) and history.json (legacy)
    ...
    def load_activities(self) -> List[Activity]:
        ...
        Prefers activities.json (new standard), falls back to history.json (legacy).

        Returns:
            List of Activity objects
        """
        # Try new standard file first
        data = self._load_json(self.activities_file)
        if data:
            return [Activity.from_dict(d) for d in data]

        # Fall back to legacy file
        data = self._load_json(self.activities_file_legacy)
        if data:
            logging.warning(
                "Loading from legacy file 'history.json'. "
                "Run scraper to update to new 'activities.json' format."
            )
            return [Activity.from_dict(d) for d in data]
