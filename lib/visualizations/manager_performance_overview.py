...
class ManagerPerformanceOverview:
    ...
    def create_performance_analysis(
        self, track_records_df: pd.DataFrame, years_window: int = None, filename: str = "performance_analysis"
    ) -> str:
    ...
    def _create_performance_chart(self, track_records_df: pd.DataFrame, analysis_period: str, filename: str) -> str:
        ...
        try:
            ...
            if is_simplified:
                ...
                gs = fig.add_gridspec(3, 3, hspace=0.45, wspace=0.4, top=0.88, bottom=0.1, left=0.08, right=0.95)
    ...
    def _create_portfolio_value_chart(self, ax, df: pd.DataFrame):
        ...
        if "current_portfolio_value" in df.columns:
            if "estimated_initial_value" in df.columns and "annualized_return_pct" in df.columns:
                ...
                _ = ax.barh(range(len(top_performers)), returns, color=colors)
                ...
                for i, (_, row) in enumerate(top_performers.iterrows()):
                    ...
                    ax.text(max_return * 1.12, i, f"${value_b:.1f}B", va="center", ha="right", fontsize=8, alpha=0.7)

            elif "estimated_initial_value" in df.columns:
                df["total_return_pct"] = (
                    (df["current_portfolio_value"] - df["estimated_initial_value"])
                    / df["estimated_initial_value"]
                    * 100
                )
                ...
                _ = ax.barh(range(len(top_performers)), top_performers["total_return_pct"])
                ax.set_yticks(range(len(top_performers)))
                ...
                colors = [
                    "darkblue" if v >= 100 else "blue" if v >= 50 else "steelblue" if v >= 10 else "lightblue"
    ...
    def _create_performance_vs_experience(self, ax, df: pd.DataFrame):
        ...
        if all(col in df.columns for col in ["years_active", "track_record_score"]):
            scatter = ax.scatter(
                ...
                s=50,
                alpha=0.7,
                c=df["current_portfolio_value"],
                cmap="viridis",
                edgecolors="black",
                linewidth=0.5,
            )
            ax.set_xlabel("Years Active", fontsize=10, fontweight="bold")
            ax.set_ylabel("Track Record Score", fontsize=10, fontweight="bold", rotation=0, ha="right", labelpad=50)
    ...
    def _create_returns_distribution(self, ax, df: pd.DataFrame):
        ...
        if "annualized_return_pct" in df.columns:
            ...
            ax.axvline(returns.mean(), color="red", linestyle="--", linewidth=2, label=f"Mean: {returns.mean():.1f}%")
    ...
    def _create_crisis_performance(self, ax, df: pd.DataFrame):
        ...
        if crisis_cols:
            ...
            ax.set_xticklabels([col.replace("_", " ").title() for col in crisis_data.index], rotation=45, ha="right")
    ...
    def _create_top_performers_table(self, ax, df: pd.DataFrame):
        ...
        for _, row in top_10.iterrows():
            ...
            portfolio_val = (
                f"${row['current_portfolio_value']/1e9:.1f}B" if pd.notna(row["current_portfolio_value"]) else "N/A"
            )
    ...
    def _create_size_vs_performance(self, ax, df: pd.DataFrame):
        ...
        if all(col in df.columns for col in ["current_portfolio_value", "annualized_return_pct"]):
            ...
            ax.scatter(
                portfolio_billions,
                valid_data["annualized_return_pct"],
                s=60,
                alpha=0.6,
                color="purple",
                edgecolors="black",
    ...
    def _create_activity_vs_returns(self, ax, df: pd.DataFrame):
        ...
        if all(col in df.columns for col in ["total_actions", "annualized_return_pct"]):
            ...
            ax.scatter(valid_data["total_actions"], valid_data["annualized_return_pct"], s=60, alpha=0.6, color="brown")
    ...
    def _create_performance_timeline(self, ax, df: pd.DataFrame):
        ...
        if all(col in df.columns for col in ["first_year", "last_year", "track_record_score"]):
            ...
            _ = ax.bar(years, active_counts, alpha=0.3, color="steelblue", label="Active Managers", width=0.8)
            ...
            ax.set_title(
                "Market Participation & Performance Evolution\n"
        ...
        if "annualized_return_pct" in df.columns and "consistency_score" in df.columns:
            ...
            if len(valid_df) > 0:
                ...
                def get_offset(x, y, labeled_positions):
                    ...
                    base_offsets = [(10, 10), (-10, 10), (10, -10), (-10, -10), (15, 0), (-15, 0), (0, 15), (0, -15)]
                ...
                for df_subset, color in [
                    (top_sharpe, "yellow"),
                    (bottom_sharpe, "lightcoral"),
                    (outliers, "lightblue"),
                ]:
                    for _, row in df_subset.iterrows():
                        ...
                        if (x, y) not in labeled_positions:
                            ...
                            ax.annotate(
                                ...
                                arrowprops=dict(arrowstyle="->", connectionstyle="arc3,rad=0.2", alpha=0.5, lw=0.5),
                            )
    ...
    def _create_cohort_analysis(self, ax, df: pd.DataFrame):
        ...
        if "first_year" in df.columns and "annualized_return_pct" in df.columns:
            ...
            _ = (
                df.groupby("cohort")
                .agg({"annualized_return_pct": ["mean", "median", "std", "count"], "track_record_score": "mean"})
                .round(2)
            )
            ...
            if cohort_data:
                ...
                ax.set_title(
                    "Performance by Manager Cohort\n(When They Started Managing)", fontsize=12, fontweight="bold"
                )
                ax.grid(True, alpha=0.3, axis="y")

                # Add median line
                medians = [data.median() for data in cohort_data]
                ax.plot(range(1, len(medians) + 1), medians, "r--", linewidth=2, label="Median Trend")
    ...
    def _create_drawdown_analysis(self, ax, df: pd.DataFrame):
        ...
        if crisis_cols and "annualized_return_pct" in df.columns:
            ...
            scatter = ax.scatter(
                valid_df["avg_crisis_buy_ratio"] * 100,
                valid_df["annualized_return_pct"],
                s=80,
                alpha=0.6,
            ...
            # Add trend line
            if len(valid_df) > 5:
                z = np.polyfit(valid_df["avg_crisis_buy_ratio"] * 100, valid_df["annualized_return_pct"], 1)
                p = np.poly1d(z)
                x_trend = np.linspace(
                    valid_df["avg_crisis_buy_ratio"].min() * 100, valid_df["avg_crisis_buy_ratio"].max() * 100, 100
                )
    ...
    def _create_top_performers_summary(self, ax, df: pd.DataFrame):
        ...
        if "annualized_return_pct" in df.columns and "consistency_score" in df.columns:
            df["composite_score"] = (
                df["annualized_return_pct"] * 0.4 + df["consistency_score"] * 100 * 0.3 + df["track_record_score"] * 0.3
            )
        ...
        for i, (_, row) in enumerate(top_5.iterrows(), 1):
            ...
            annual_return = (
                f"{row['annualized_return_pct']:.1f}%" if pd.notna(row.get("annualized_return_pct")) else "N/A"
            )
