...
class DataValidator:
    ...
    def validate_activities_data(self) -> bool:
        ...
        try:
            ...
            # Check required fields in all records (or a representative sample for large datasets)
            required_fields = ["ticker", "manager_id", "action_type", "period"]
            # Check up to first 100 records to balance thoroughness with performance
            sample_size = min(100, len(activities))
            for i, activity in enumerate(activities[:sample_size]):
                for field in required_fields:
                    if field not in activity:
                        logger.error(f"Missing required field '{field}' in activity record {i}")
                        return False
    ...
    def validate_holdings_data(self) -> bool:
        ...
        try:
            ...
            # Check up to first 100 records to balance thoroughness with performance
            sample_size = min(100, len(holdings))
            for i, holding in enumerate(holdings[:sample_size]):
                for field in required_fields:
                    if field not in holding:
                        logger.error(f"Missing required field '{field}' in holding record {i}")
                        return False
