...
class DataromaParser:
        ...
        for link in soup.find_all("a", href=True):
            if isinstance(link, Tag):
                ...
                if "/m/holdings.php?m=" in href:
                    # Extract and URL-decode manager_id, strip any URL fragments
                    manager_id = href.split("m=")[-1].split("#")[0]
                    manager_id = unquote(manager_id)
        ...
        if not table:
            ...
            for t in tables:
                if isinstance(t, Tag):
                    ...
                    if any(word in header_text for word in ["stock", "shares", "value", "portfolio"]):
                        table = t
                        break
            ...
            for row in rows:
                ...
                if len(cells) >= 12:  # Need all columns including 52-week data
                    try:
                        ...
                        logging.warning(f"Failed to parse holding row for {manager_id}: {e}")
                        continue
            ...
            if element.name == "td":
                ...
                if len(activity_cells) >= 5:
                    try:
                        ...
                        activity = self._parse_activity_cells(activity_cells, manager_id, current_quarter, current_year)
                        if activity:
    ...
    def _parse_activity_cells(self, cells: List[Tag], manager_id: str, quarter: str, year: str) -> Optional[Activity]:
    ...
    def _parse_holding_row(
        self, cells: List[Tag], manager_id: str, reporting_date: str, quarter: str
    ) -> Optional[Holding]:
        ...
        try:
            # Check if first cell is history link (use membership check for robustness)
            has_history_cell = "hist" in (cells[0].get("class") or [])
    ...
    def _parse_currency(self, text: str) -> float:
        """Parse currency from text, including parenthetical negatives like ($50.00)."""
        if not text:
            return 0.0

        text_str = str(text).strip()

        # Check for parenthetical negative format: ($50.00) or (50.00)
        is_negative = text_str.startswith("(") and text_str.endswith(")")
        if is_negative:
            text_str = text_str[1:-1]  # Remove parentheses
