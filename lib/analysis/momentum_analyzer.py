...
class MomentumAnalyzer(MultiAnalyzer):
        ...
        if self.data.holdings_df is not None and not self.data.holdings_df.empty:
            ...
            if len(available_cols) >= 2:
                ...
                price_data["near_52w_low"] = price_data.apply(
                    lambda row: self.calc.is_near_52_week_low(row["current_price"], row["52_week_low"], 15.0), axis=1
                )
        ...
        if self.data.holdings_df is not None and not self.data.holdings_df.empty:
            ...
            if len(available_cols) >= 2:
                ...
                price_data["near_52w_high"] = price_data.apply(
                    lambda row: self.calc.is_near_52_week_high(row["current_price"], row["52_week_high"], 15.0), axis=1
                )
    ...
    def analyze_most_sold_stocks(self) -> pd.DataFrame:
        ...
        all_sells = self.data.history_df[self.data.history_df["action_type"].isin(["Sell", "Reduce"])].copy()
        ...
        def calculate_shares_sold(group):
            ...
            for _, row in group.iterrows():
                if row["action_type"] == "Sell":
                    # For full sells, use the shares value (shares before selling)
                    total_sold += row.get("shares", 0)
                elif row["action_type"] == "Reduce" and "action" in row:
                    # Parse percentage from "Reduce XX.XX%"
                    match = re.search(r"(\d+\.?\d*)%", str(row["action"]))
                    if match and "shares" in row:
                        reduce_pct = float(match.group(1)) / 100
                        shares_after = row["shares"]
                        # Calculate shares before reduction: shares_after = shares_before * (1 - reduce_pct)
                        # So shares_before = shares_after / (1 - reduce_pct)
                        shares_before = shares_after / (1 - reduce_pct) if reduce_pct < 1 else shares_after
                        shares_sold = shares_before - shares_after
                        total_sold += shares_sold
            return total_sold

        shares_sold = all_sells.groupby("ticker").apply(calculate_shares_sold).rename("shares_sold")
        ...
        sell_analysis["exit_rate_pct"] = (
            sell_analysis["unique_sellers"]
            / (sell_analysis["unique_sellers"] + sell_analysis["remaining_holders"])
            * 100
        ).fillna(100)
