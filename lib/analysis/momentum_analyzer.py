...
class MomentumAnalyzer(MultiAnalyzer):
    ...
    def analyze_momentum_stocks(self) -> pd.DataFrame:
        ...
        if recent_quarters and len(recent_quarters) >= 2:
            ...
            momentum.loc[
                (momentum["buy_count"] >= 3) & (momentum["periods"].str.contains(recent_pattern, na=False)),
                "momentum_type",
            ] = "Recent Surge"
        ...
        result = self.format_output(momentum.reset_index()).head(50)
        return self.add_metadata_columns(result, window_quarters=3, analysis_type="momentum_stocks")
    ...
    def analyze_new_positions(self) -> pd.DataFrame:
        ...
        new_positions = self.data.history_df[
            (self.data.history_df["action_type"] == "Buy") & (self.data.history_df["period"].isin(recent_quarters))
        ].copy()
        ...
        # Add manager names
        new_analysis["manager_name"] = new_analysis["manager_id"].map(lambda x: self.data.manager_names.get(x, x))
        ...
        if self.data.holdings_df is not None and "stock" in self.data.holdings_df.columns:
            ...
            new_analysis = new_analysis.set_index("ticker").join(company_names.rename("company_name")).reset_index()
        ...
        if self.data.holdings_df is not None and not self.data.holdings_df.empty:
            ...
            new_analysis = (
                new_analysis.set_index(["ticker", "manager_id"])
                .join(current_status, how="left", rsuffix="_current")
                .reset_index()
            )
    ...
    def analyze_52_week_low_buys(self) -> pd.DataFrame:
            ...
            if len(available_cols) >= 2:
                ...
                price_data = self.data.holdings_df.groupby("ticker").agg(
                    {
                        ...
                        "value": "sum",
                        "portfolio_percent": (
                            "mean" if "portfolio_percent" in self.data.holdings_df.columns else "count"
                        ),
                        "stock": "first" if "stock" in self.data.holdings_df.columns else "count",
                    }
                )
                ...
                if "52_week_high" in price_data.columns:
                    ...
                    price_data["52_week_position_pct"] = (
                        (price_data["current_price"] - price_data["52_week_low"]) / price_data["52_week_low"] * 100
                    ).clip(0, 100)

                # Identify near-low positions (bottom 25% of 52-week range)
                ...
                low_buys = buy_summary[
                    buy_summary["near_52w_low"].eq(True)
                ...
                if not low_buys.empty:
                    ...
                    low_buys["value_opportunity_score"] = (
                        low_buys["buy_count"] * 2
                        + (100 - low_buys["52_week_position_pct"]) / 10  # More buys = better
                        + low_buys["value"] / 1000000  # Lower position = better  # Position size factor
                    )

                    # Categorize opportunity type
                    low_buys["opportunity_type"] = "Value Buying"
                    low_buys.loc[low_buys["52_week_position_pct"] < 10, "opportunity_type"] = "Deep Value"
                    low_buys.loc[low_buys["buy_count"] >= 5, "opportunity_type"] = "Strong Accumulation"

                    # Sort by value opportunity score
                    low_buys = low_buys.sort_values("value_opportunity_score", ascending=False)

                    result = self.format_output(low_buys.reset_index()).head(40)
                    return self.add_metadata_columns(result, window_quarters=3, analysis_type="52_week_low_buys")
        ...
        result = self.format_output(buy_summary.reset_index()).head(30)
        return self.add_metadata_columns(result, window_quarters=3, analysis_type="52_week_low_buys")
    ...
    def analyze_52_week_high_sells(self) -> pd.DataFrame:
        ...
        if self.data.holdings_df is not None and not self.data.holdings_df.empty:
            ...
            if len(available_cols) >= 2:
                ...
                if "52_week_low" in price_data.columns:
                    ...
                    # Just use high data (use DataFrame columns, not undefined 'row')
                    price_data["52_week_position_pct"] = (
                        price_data["current_price"] / price_data["52_week_high"] * 100
                    ).clip(0, 100)

                # Identify near-high positions (top 25% of 52-week range)
                ...
                high_sells = sell_summary[
                    sell_summary["near_52w_high"].eq(True)
                ...
                if not high_sells.empty:
                    ...
                    high_sells["profit_taking_score"] = (
                        high_sells["sell_count"] * 2
                        + high_sells["52_week_position_pct"] / 10  # More sells = more conviction
                        + high_sells["value"] / 1000000  # Higher position = better timing  # Position size factor
                    )

                    # Categorize selling type
                    high_sells["selling_type"] = "Profit Taking"
                    high_sells.loc[high_sells["52_week_position_pct"] > 90, "selling_type"] = "Peak Selling"
                    high_sells.loc[high_sells["sell_count"] >= 5, "selling_type"] = "Heavy Distribution"

                    # Sort by profit-taking score
                    high_sells = high_sells.sort_values("profit_taking_score", ascending=False)

                    result = self.format_output(high_sells.reset_index()).head(40)
                    return self.add_metadata_columns(result, window_quarters=3, analysis_type="52_week_high_sells")
        ...
        result = self.format_output(sell_summary.reset_index()).head(30)
        return self.add_metadata_columns(result, window_quarters=3, analysis_type="52_week_high_sells")

    def analyze_most_sold_stocks(self) -> pd.DataFrame:
        """
        Find stocks with the most selling activity in recent quarters.

        Returns:
            DataFrame with heavily sold stocks (last 3 quarters only)
        """
        if (
            self.data.history_df is None
            or self.data.history_df.empty
        ...
        # CRITICAL: Filter to recent quarters only for "current" classification
        recent_quarters = self.get_recent_quarters(3)
        logging.info(f"Filtering most_sold_stocks to recent quarters: {recent_quarters}")

        # Get sell activities from recent quarters ONLY
        ...
        sell_analysis = all_sells.groupby("ticker").agg(
            {
                "manager_id": ["nunique", self.get_manager_summary],  # Changed count to nunique for unique managers
        ...
        def calculate_shares_sold(group):
            """Calculate total shares sold for a group of transactions.

            IMPORTANT DATA MODEL:
            The 'shares' field in history data represents SHARES CHANGED in each transaction,
            NOT the cumulative position. For example:
            - Buy: shares = initial purchase amount
            - Add X%: shares = shares added (≈ previous_cumulative × X%)
            - Reduce X%: shares = shares sold (≈ previous_cumulative × X%)
            - Sell 100%: shares = total position sold
            """
            total_sold = 0
            for _, row in group.iterrows():
                if row["action_type"] == "Sell":
                    # For sells, shares = shares sold in this transaction
                    total_sold += row.get("shares", 0)
                elif row["action_type"] == "Reduce":
                    # For reduces, shares = shares sold in this transaction
                    # No calculation needed - the data already represents shares sold
                    total_sold += row.get("shares", 0)
            return total_sold

        shares_sold = all_sells.groupby("ticker").apply(calculate_shares_sold, include_groups=False).rename("shares_sold")
        sell_analysis = sell_analysis.join(shares_sold)
        ...
        # Calculate exit metrics - FIXED: using unique sellers, not total actions
        # exit_rate = sellers / (sellers + remaining_holders)
        # If no remaining holders and we have sellers, that's 100% exit
        # If both are 0 or NaN, set to 0 (unknown, not 100%)
        total_participants = sell_analysis["unique_sellers"] + sell_analysis["remaining_holders"]
        sell_analysis["exit_rate_pct"] = (
            sell_analysis["unique_sellers"] / total_participants.replace(0, float("nan")) * 100
        ).fillna(0)  # Unknown defaults to 0%, not 100%

        # If we have sellers but 0 remaining holders, that IS 100% exit
        mask_full_exit = (sell_analysis["unique_sellers"] > 0) & (sell_analysis["remaining_holders"] == 0)
        sell_analysis.loc[mask_full_exit, "exit_rate_pct"] = 100.0

        # Add total_sells column BEFORE converting dict to string
        sell_analysis["total_sells"] = sell_analysis["action_breakdown"].apply(lambda x: sum(x.values()))

        # Replace action_breakdown dict with readable string format
        sell_analysis["action_breakdown"] = sell_analysis["action_breakdown"].apply(
        ...
        # Filter for meaningful selling activity (at least 2 unique sellers)
        meaningful_sells = sell_analysis[sell_analysis["unique_sellers"] >= 2].copy()

        # Sort by unique sellers and exit rate
        meaningful_sells = meaningful_sells.sort_values(["unique_sellers", "exit_rate_pct"], ascending=[False, False])

        result = self.format_output(meaningful_sells.reset_index()).head(50)
        return self.add_metadata_columns(result, window_quarters=3, analysis_type="most_sold_stocks")
    ...
    def analyze_concentration_changes(self) -> pd.DataFrame:
        ...
        if (
            ...
            or self.data.history_df is None
        ...
        high_concentration = self.data.holdings_df[self.data.holdings_df["portfolio_percent"] > 3.0].copy()
        ...
        concentration_summary = (
            high_concentration.groupby(["ticker", "manager_id"])
            .agg(
                {
        ...
        if "action_type" in self.data.history_df.columns:
            recent_activity = self.data.history_df[self.data.history_df["period"].isin(self.get_recent_quarters(3))]

            if not recent_activity.empty:
                # Get recent actions for these positions
                activity_by_position = (
                    recent_activity.groupby(["ticker", "manager_id"])
                    .agg(
                        {
                ...
                activity_by_position.columns = ["ticker", "manager_id", "recent_actions", "action_details", "periods"]
                ...
                concentration_summary.loc[
                    concentration_summary["recent_actions"].str.contains("Add", na=False), "change_type"
                ] = "Increased"
                concentration_summary.loc[
                    concentration_summary["recent_actions"].str.contains("Buy", na=False), "change_type"
                ] = "New High Conviction"
                concentration_summary.loc[
                    concentration_summary["recent_actions"].str.contains("Reduce", na=False), "change_type"
                ] = "Reduced"
                concentration_summary.loc[
                    concentration_summary["recent_actions"].str.contains("Sell", na=False), "change_type"
                ] = "Partial Exit"
        ...
        if "change_type" in concentration_summary.columns:
            ...
            concentration_summary.loc[concentration_summary["change_type"] == "Increased", "concentration_score"] *= 1.5
            concentration_summary.loc[
        ...
        result = self.format_output(concentration_summary).head(100)
        return self.add_metadata_columns(result, window_quarters=3, analysis_type="concentration_changes")
