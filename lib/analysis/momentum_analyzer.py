...
class MomentumAnalyzer(MultiAnalyzer):
    ...
    def analyze_most_sold_stocks(self) -> pd.DataFrame:
        ...
        sell_analysis = all_sells.groupby("ticker").agg({
            ...
            "action_type": lambda x: {k: int(v) for k, v in dict(x.value_counts()).items()},  # Convert numpy int64 to Python int
        ...
        sell_analysis["exit_rate_pct"] = (
            ...
            (sell_analysis["unique_sellers"] + sell_analysis["remaining_holders"]) * 100
        ).fillna(100)

        # Add total_sells column BEFORE converting dict to string
        sell_analysis["total_sells"] = sell_analysis["action_breakdown"].apply(
        ...
        meaningful_sells = meaningful_sells.sort_values(
            ["unique_sellers", "exit_rate_pct"], ascending=[False, False]
        )
