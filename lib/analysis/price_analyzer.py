...
class PriceAnalyzer(MultiAnalyzer):
    ...
    def analyze_stocks_under_price(self, max_price: float) -> pd.DataFrame:
        ...
        if "current_price" not in analysis_df.columns:
            ...
            if "shares" in analysis_df.columns and "value" in analysis_df.columns:
                analysis_df["current_price"] = (analysis_df["value"] / analysis_df["shares"]).fillna(0)
            else:
        ...
        if (
            ...
            recent_activity.columns = ["recent_buys", "latest_buy_quarter"]
            recent_activity = recent_activity.reset_index()
            grouped = grouped.merge(recent_activity, on="ticker", how="left")
            grouped["recent_buys"] = grouped["recent_buys"].fillna(0)

            # CRITICAL: Filter to only stocks with recent activity for "current" classification
            # Stocks without recent buys (like KW with last buy in Q3 2020) should not appear
            # in current/ folder files - they represent stale holdings, not current opportunities
            grouped = grouped[grouped["recent_buys"] > 0].copy()
            logging.info(f"Filtered stocks_under_${max_price} to {len(grouped)} stocks with recent activity")

            if grouped.empty:
                logging.warning(f"No stocks under ${max_price} with recent activity")
                return pd.DataFrame()

        # Calculate price opportunity score
        grouped["price_opportunity_score"] = (
            grouped["manager_count"] * 2
            + grouped["avg_portfolio_pct"]  # More managers = better
            + grouped.get("recent_buys", 0)  # Higher conviction = better  # Recent activity = better
        )
        ...
        result = self.format_output(meaningful_positions.reset_index()).head(50)
        return self.add_metadata_columns(result, window_quarters=3, analysis_type=f"stocks_under_${int(max_price)}")
    ...
    def analyze_high_conviction_low_price(self) -> pd.DataFrame:
        ...
        """
        if not self.validate_required_columns(self.data.holdings_df, ["ticker", "portfolio_percent"]):
            return pd.DataFrame()
        ...
        high_conviction_low_price = holdings[
            (holdings["portfolio_percent"] > 5.0) & (holdings["current_price"] > 0) & (holdings["current_price"] < 25.0)
        ].copy()
        ...
        grouped["conviction_price_score"] = (
            grouped["max_portfolio_pct"] * 2
            + (25 - grouped["current_price"])  # Higher conviction = better
            + grouped["manager_count"]  # Lower price = better  # More managers = better
        )

        # Add recent activity filter for "current" classification
        if (
            self.data.history_df is not None
            and not self.data.history_df.empty
            and "action_type" in self.data.history_df.columns
            ...
            grouped = grouped.merge(recent_buys, on="ticker", how="left")
            grouped["recent_buys"] = grouped["recent_buys"].fillna(0)

            # CRITICAL: Filter to only stocks with recent activity for "current" classification
            grouped = grouped[grouped["recent_buys"] > 0].copy()
            logging.info(f"Filtered high_conviction_low_price to {len(grouped)} stocks with recent activity")

            if grouped.empty:
                logging.warning("No high conviction low price stocks with recent activity")
                return pd.DataFrame()

        # Categorize opportunities
        grouped["opportunity_type"] = "High Conviction Low Price"
        grouped.loc[(grouped["max_portfolio_pct"] > 10) & (grouped["current_price"] < 10), "opportunity_type"] = (
            "Deep Conviction Bargain"
        )
        grouped.loc[grouped["manager_count"] >= 3, "opportunity_type"] = "Multi-Manager Conviction"

        # Sort by conviction-price score
        grouped = grouped.sort_values(by="conviction_price_score", ascending=False)

        result = self.format_output(grouped.reset_index()).head(30)
        return self.add_metadata_columns(result, window_quarters=3, analysis_type="high_conviction_low_price")
    ...
    def analyze_value_price_opportunities(self) -> pd.DataFrame:
        ...
        value_analysis["price_change_pct"] = (
            ...
            / value_analysis["avg_reported_price"]
            * 100
        ).fillna(0)
        ...
        if "52_week_low" in holdings.columns:
            ...
            value_analysis["discount_to_52w_low_pct"] = (
                (value_analysis["current_price"] - value_analysis["52_week_low"]) / value_analysis["52_week_low"] * 100
            ).fillna(100)
        else:
        ...
        if (
            ...
            # CRITICAL: Filter to only stocks with recent activity for "current" classification
            value_analysis = value_analysis[value_analysis["recent_buy_count"] > 0].copy()
            logging.info(f"Filtered value_price_opportunities to {len(value_analysis)} stocks with recent activity")

            if value_analysis.empty:
                logging.warning("No value opportunities with recent activity")
                return pd.DataFrame()
        ...
        value_analysis["value_opportunity_score"] += value_analysis["price_change_pct"].apply(lambda x: max(0, -x) / 5)

        # Factor 2: Proximity to 52-week low (closer = better)
        value_analysis["value_opportunity_score"] += (100 - value_analysis["discount_to_52w_low_pct"]) / 20

        # Factor 3: Manager conviction
        value_analysis["value_opportunity_score"] += value_analysis["avg_portfolio_pct"] / 2

        # Factor 4: Recent activity
        if "recent_buy_count" in value_analysis.columns:
            value_analysis["value_opportunity_score"] += value_analysis["recent_buy_count"]

        # Factor 5: Manager count
        value_analysis["value_opportunity_score"] += value_analysis["manager_count"]

        # Filter for meaningful opportunities (score > 5)
        value_opportunities = value_analysis[value_analysis["value_opportunity_score"] > 5].copy()
        ...
        value_opportunities.loc[value_opportunities["price_change_pct"] < -10, "value_type"] = "Price Discount"
        value_opportunities.loc[value_opportunities["discount_to_52w_low_pct"] < 20, "value_type"] = "Near 52W Low"
        value_opportunities.loc[
            (value_opportunities["recent_buy_count"] > 3) & (value_opportunities["avg_portfolio_pct"] > 3), "value_type"
        ] = "Active Accumulation"

        # Sort by value opportunity score
        value_opportunities = value_opportunities.sort_values(by="value_opportunity_score", ascending=False)

        result = self.format_output(value_opportunities.reset_index()).head(40)
        return self.add_metadata_columns(result, window_quarters=3, analysis_type="value_price_opportunities")
