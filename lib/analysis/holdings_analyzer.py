...
class HoldingsAnalyzer(MultiAnalyzer):
    ...
    def analyze_multi_manager_favorites(self) -> pd.DataFrame:
        ...
        if (
            ...
            # Calculate earliest_date (first activity) and action_date (most recent activity)
            date_data = self.data.history_df.groupby("ticker")["period"].agg(earliest_date="min", action_date="max")
            consensus_picks = consensus_picks.join(date_data, how="left")
    ...
    def analyze_interesting_stocks_overview(self) -> pd.DataFrame:
        ...
        if (
            ...
            # Calculate action_date (most recent activity)
            action_dates = self.data.history_df.groupby("ticker")["period"].max().rename("action_date")
            overview = overview.join(action_dates, how="left")
    ...
    def analyze_high_conviction_stocks(self) -> pd.DataFrame:
        ...
        # Flatten columns
        col_names = ["manager_count", "managers", "avg_portfolio_pct", "max_portfolio_pct", "total_value"]
