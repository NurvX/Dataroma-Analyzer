...
class DataLoader:
    ...
    def _load_activities(self) -> None:
        ...
        if not activities_path.exists():
            logging.warning("No activities file found")
            return
        ...
        # Add action_type extraction ONLY if not already present in JSON
        if "action" in self.history_df.columns and "action_type" not in self.history_df.columns:
            self.history_df["action_type"] = self.history_df["action"].apply(self._extract_action_type)
    ...
    def _add_manager_names(self) -> None:
        ...
        if self.holdings_df is not None and not self.holdings_df.empty:
            ...
            self.holdings_df["manager_name"] = self.holdings_df[manager_col].map(lambda x: self.manager_names.get(x, x))
        ...
        if self.history_df is not None and not self.history_df.empty:
            ...
            self.history_df["manager_name"] = self.history_df[manager_col].map(lambda x: self.manager_names.get(x, x))

    def _extract_action_type(self, action: Any) -> str:
        ...
        # Check for Sell actions (including "Sell X%" format)
        if "sell" in action_lower or "sold all" in action_lower or "sold out" in action_lower or "exit" in action_lower:
            return "Sell"
