...
def create_all_visualizations(results: dict, output_dir: str = "analysis"):
    ...
    try:
        ...
        historical_results = {
            ...
            if any(
                keyword in name
                for keyword in ["historical", "crisis_response", "multi_decade", "quarterly", "life_cycle", "long_term"]
            )
            and not df.empty
        }
        ...
        try:
            ...
            historical_results = {
                ...
                for name, df in results.items()
                if any(
                    keyword in name for keyword in ["historical", "crisis", "multi_decade", "quarterly", "life_cycle"]
                )
                and not df.empty
            }
            if historical_results:
...
def create_visualizations(results: dict, output_dir: str = "analysis"):
    ...
    try:
        ...
        historical_results = {
            ...
            if any(
                keyword in name
                for keyword in ["historical", "crisis", "multi_decade", "quarterly", "life_cycle", "track_record"]
            )
            and not df.empty
        }
